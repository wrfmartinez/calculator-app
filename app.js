...
for (number of numbers) {
  number.addEventListener('click', (event) => {
    ...
    if (event.target.textContent === '=') {
      const displayAddition = display.textContent = add(storedNumbers.storedNumbersToAdd);
      const displaySubraction = display.textContent = subtract(storedNumbers.storedNumbersToSubtract);
      const displayMultiplication = display.textContent = multiply(storedNumbers.storedNumbersToMultiply);
      const displayDivison = display.textContent = divide(storedNumbers.storedNumbersToDivide);
